...
namespace ClinicServiceTest
{
    public class ClientControllerTests
    {
        ...
        {
            _mocClientRepository.Setup(repository => repository.Delete(0)).Returns(1).Verifiable();
            var operationResult = _clientController.Delete(0);

            Assert.IsType<OkObjectResult>(operationResult.Result);
            Assert.IsAssignableFrom<int>(((OkObjectResult)operationResult.Result).Value);
            _mocClientRepository.Verify(repository =>
                repository.Delete(0), Times.AtLeastOnce());
        }
