...
namespace ClinicService.Controllers
{
    ...
    public class PetController : Controller
    {
        ...
        [HttpGet("get/{petId}")]
        [SwaggerOperation(OperationId = "PetGetAllByPetId")]
        public ActionResult<Pet> GetById([FromRoute] int petId)
        {
            return Ok(_petRepository.GetById(petId));
        }

        [HttpGet("gets/{clientId}")]
        [SwaggerOperation(OperationId = "PetGetAllByClientId")]
        public ActionResult<List<Pet>> GetByClientId([FromRoute] int clientId)
        {
